...
export function Dashboard({ user, transactions, goals, addGoal, removeGoal, toggleRecurring, recalculateGoal, onUpgrade }: { user: UserProfile, transactions: Transaction[], goals: Goal[], addGoal?: any, removeGoal?: any, toggleRecurring?: any, recalculateGoal?: any, onUpgrade?: () => void }) {
  ...
  const getFilteredForReport = () => {
    ...
    return { filtered, periodLabel, periodGoals };
  };

  const handleDownloadReport = (action: 'save' | 'print' = 'save') => {
    const { filtered, periodLabel, periodGoals } = getFilteredForReport();
    generateReportPDF(filtered, periodLabel, user.email, action, periodGoals);
  };

  const handleExportCSV = () => {
    const { filtered, periodLabel } = getFilteredForReport();
    exportTransactionsToCSV(filtered, periodLabel);
  };
        ...
        {activeTab === 'liberty' ? (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div onClick={() => {
                if (!user.isPro) { alert("Los detalles de gastos son una función Pro."); return; }
                setShowExpenseDetails(true);
              }} className="cursor-pointer">
                <StatCard label="Total Gastos" amount={totalExpenses} icon={ArrowDownRight} color="text-red-500" bg="bg-red-50" trend={expenseTrend} privacyMode={privacyMode} interactive />
              </div>
              <div onClick={() => {
                if (!user.isPro) { alert("Los detalles de ingresos son una función Pro."); return; }
                setShowIncomeDetails(true);
              }} className="cursor-pointer">
                <StatCard label="Total Ingresos" amount={totalIncome} icon={ArrowUpRight} color="text-green-500" bg="bg-green-50" trend={incomeTrend} privacyMode={privacyMode} interactive />
              </div>
              <StatCard label="Balance Neto" amount={balance} icon={TrendingUp} color="text-blue-500" bg="bg-blue-50" trend={balance >= 0 ? 'Positivo' : 'Déficit'} privacyMode={privacyMode} />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="md:col-span-2">
                <AIAdvisor user={user} transactions={transactions} goals={goals} onUpgrade={onUpgrade} />
              </div>
              <div>
                <SubscriptionManager user={user} transactions={transactions} toggleRecurring={toggleRecurring} onUpgrade={onUpgrade} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="h-80 mt-4">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={chartData} layout="vertical" margin={{ top: 5, right: 50, left: 20, bottom: 5 }}>
                      ...
                      <Tooltip contentStyle={{ backgroundColor: '#fff', borderRadius: '16px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)', color: '#0f172a' }} itemStyle={{ color: '#0f172a' }} formatter={(value: number) => formatHidable(value)} />
                      <Bar dataKey="amount" radius={[0, 4, 4, 0]} barSize={24}>
                        {chartData.map((_, index) => (
                          <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                        ))}
                        <LabelList dataKey="amount" position="right" formatter={(value: number) => formatHidable(value)} style={{ fill: '#475569', fontSize: 10, fontWeight: '600' }} offset={10} />
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              ...
              <div className="glass p-8 rounded-[2.5rem]">
                ...
                <div className="space-y-6">
                    ...
                    <div key={goal.id} className="space-y-2 group relative">
                      <div className="flex justify-between items-center text-sm">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex items-center gap-1">
                            <button onClick={() => recalculateGoal?.(goal.id!)} className="p-1.5 text-slate-400 hover:text-blue-500 hover:bg-blue-50 rounded-lg transition-all" title="Recalcular presupuesto">
                              <RefreshCw className="w-3.5 h-3.5" />
                            </button>
                            <AnimatePresence mode="wait">
                              {goalToDelete === goal.id ? (
                                <motion.div key="confirm" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="flex items-center gap-1 bg-red-50 rounded-lg p-1 border border-red-100">
                                  <button type="button" onClick={() => handleDeleteGoal(goal.id!)} className="text-[10px] font-bold text-red-600 px-2 py-1 hover:bg-red-100 rounded-md transition-colors">Eliminar</button>
                                  <button type="button" onClick={() => setGoalToDelete(null)} className="text-[10px] font-bold text-slate-400 px-2 py-1 hover:bg-slate-100 rounded-md transition-colors">No</button>
                                </motion.div>
                              ) : (
                                <motion.button key="delete-btn" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setGoalToDelete(goal.id!)} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all" title="Eliminar presupuesto">
                      ...
                      <div className="w-full bg-slate-100 h-3 rounded-full overflow-hidden">
                        <motion.div 
                          ...
                          className={cn("h-full rounded-full transition-all duration-1000", ((goal.currentAmount || 0) / goal.targetAmount) >= 1 ? "bg-red-500" : ((goal.currentAmount || 0) / goal.targetAmount) >= 0.8 ? "bg-orange-500" : "bg-blue-600")}
                        />
                      </div>
                  ...
                  <button type="button" onClick={() => setShowGoalForm(true)} className="w-full py-4 border-2 border-dashed border-slate-200 rounded-2xl text-slate-400 hover:border-blue-500 hover:text-blue-500 transition-all font-medium text-sm">
                    + Nuevo Presupuesto
                  </button>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 gap-6">
              <div className="glass p-8 rounded-[2.5rem] transition-colors">
                <div className="flex items-center justify-between mb-8">
                  ...
                  {!user.isPro && <span className="text-[10px] font-bold text-amber-600 bg-amber-50 px-3 py-1 rounded-full uppercase border border-amber-100">Solo Pro</span>}
                </div>
                <div className="h-64 mt-4 relative">
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={historyData}>
                      ...
                      <Tooltip formatter={(val: number) => formatHidable(val)} contentStyle={{ backgroundColor: '#fff', borderRadius: '16px', border: 'none', boxShadow: '0 10px 15px -10px rgb(0 0 0 / 0.1)', color: '#0f172a' }} />
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-500">Genera documentos PDF y CSV de tus movimientos.</p>
                  </div>
                </div>
                <button 
                  type="button"
      ...
      <AnimatePresence>
        {showExpenseDetails && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="glass max-w-sm w-full p-8 rounded-[2.5rem]">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold">Detalle de Gastos</h3>
                <button onClick={() => setShowExpenseDetails(false)} className="p-2 hover:bg-slate-100 rounded-full transition-colors"><X className="w-5 h-5 text-slate-400" /></button>
              </div>
              <div className="space-y-6">
                <div className="flex gap-2">
                  {(['today', 'week', 'total'] as const).map(f => (
                    <button key={f} onClick={() => setExpenseFilter(f)} className={cn("flex-1 py-2 rounded-xl text-xs font-bold transition-all", expenseFilter === f ? "bg-blue-600 text-white shadow-lg shadow-blue-600/30" : "bg-slate-100 text-slate-500")}>
                ...
                <div className="p-6 bg-slate-50 rounded-3xl text-center border border-slate-100 transition-colors">
                  <p className="text-sm text-slate-500 mb-1">{expenseFilter === 'today' ? 'Gastos de Hoy' : expenseFilter === 'week' ? 'Gastos de la Semana' : 'Gastos Totales'}</p>
                  <p className="text-3xl font-bold text-slate-900">{formatCurrency(getFilteredExpenses(expenseFilter).reduce((acc, t) => acc + t.amount, 0))}</p>
                </div>
                <div className="space-y-3 max-h-48 overflow-y-auto pr-2">
                  {getFilteredExpenses(expenseFilter).slice(0, 5).map((t) => (
                    <div key={t.id} className="flex items-center justify-between p-3 bg-white border border-slate-100 rounded-xl transition-colors">
                      <div><p className="text-xs font-bold text-slate-900">{t.description}</p><p className="text-[10px] text-slate-500">{t.category}</p></div>
                      <span className="text-xs font-bold text-red-500">{formatCurrency(t.amount)}</span>
                    </div>
                  ))}
                  {getFilteredExpenses(expenseFilter).length === 0 && <p className="text-center text-xs text-slate-400 py-4">No hay gastos para este periodo.</p>}
                </div>
                <button onClick={() => setShowExpenseDetails(false)} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-bold active:scale-95 transition-all">Cerrar</button>
              </div>
            </motion.div>
          </div>
        )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="glass max-w-sm w-full p-8 rounded-[2.5rem]">
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setShowIncomeDetails(false)} className="p-2 hover:bg-slate-100 rounded-full transition-colors"><X className="w-5 h-5 text-slate-400" /></button>
              </div>
              <div className="space-y-6">
                <div className="flex gap-2">
                  {(['today', 'week', 'total'] as const).map((f) => (
                    <button key={f} onClick={() => setIncomeFilter(f)} className={cn("flex-1 py-2 rounded-xl text-xs font-bold transition-all", incomeFilter === f ? "bg-blue-600 text-white shadow-lg shadow-blue-600/30" : "bg-slate-100 text-slate-500")}>
                      {f === 'today' ? 'Hoy' : f === 'week' ? 'Semana' : 'Todo'}
                    </button>
                  ))}
                </div>
                <div className="p-6 bg-slate-50 rounded-3xl text-center border border-slate-100 transition-colors">
                  <p className="text-sm text-slate-500 mb-1">{incomeFilter === 'today' ? 'Ingresos de Hoy' : incomeFilter === 'week' ? 'Ingresos de la Semana' : 'Ingresos Totales'}</p>
                  <p className="text-3xl font-bold text-slate-900">{formatCurrency(getFilteredIncomes(incomeFilter).reduce((acc, t) => acc + t.amount, 0))}</p>
                </div>
                <div className="space-y-3 max-h-48 overflow-y-auto pr-2">
                  ...
                  {getFilteredIncomes(incomeFilter).length === 0 && <p className="text-center text-xs text-slate-400 py-4">No hay ingresos para este periodo.</p>}
                </div>
                <button onClick={() => setShowIncomeDetails(false)} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-bold active:scale-95 transition-all">Cerrar</button>
              </div>
            </motion.div>
          </div>
        )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="glass max-w-sm w-full p-8 rounded-[2.5rem]">
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setShowGoalForm(false)} className="p-2 hover:bg-slate-100 rounded-full transition-colors"><X className="w-5 h-5 text-slate-400" /></button>
              </div>
              <form onSubmit={handleAddGoal} className="space-y-4">
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Mes y Año</label>
                  <div className="grid grid-cols-2 gap-2">
                    <select value={newGoal.month} onChange={e => setNewGoal({ ...newGoal, month: parseInt(e.target.value) })} className="w-full p-4 bg-slate-100 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500 transition-colors">
                      {MONTHS.map((m, i) => <option key={m} value={i + 1}>{m}</option>)}
                    </select>
                    <select value={newGoal.year} onChange={e => setNewGoal({ ...newGoal, year: parseInt(e.target.value) })} className="w-full p-4 bg-slate-100 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500 transition-colors">
